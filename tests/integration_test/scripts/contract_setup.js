...
module.exports = async (app, admin, controller, master, nft, universal) => {
	...
	app.controller_contract = truffle_contract(
		require(getPath() + "ethereum/build/contracts/Controller.json")
	);
	...
	change_controller(controller, (res_code, res_msg) => {
		if (res_code !== 200) {
			...
			app.changeController = contract_endpoints.changeController(
				admin,
				controller
			);
			...
			app.safeTransferFrom = contract_endpoints.safeTransferFrom(
				admin,
				universal
			);
			app.setApprovalForAll = contract_endpoints.setApprovalForAll(
				admin,
				universal
			);
